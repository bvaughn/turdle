...
const { enterWord, isKeyEnabled, pressKey, takeKeyboardScreenshot } = require("./utils/keyboard");
const { takeEndGameModalScreenshot, takeInvalidGuessModalScreenshot } = require("./utils/modals");
...
test.describe("game", () => {
  ...
  test.describe("input", () => {
    test("enter and delete keys should be disabled until a guess has been entered", async ({ page }) => {
    ...
    test("should accept input from the on-screen keyboard", async ({ page }) => {
  ...
  test("should only accept guesses that are valid 4-letter poop words", async ({ page }) => {
    ...
    await expect(page.locator('[data-testname="InvalidGuessModal"]')).toBeHidden();
  });
  ...
  test("should show no-more-words modal when word list is empty", async ({ page }) => {
  ...
  test("should track win/loss statistics and number of guesses across games", async ({ page }) => {
